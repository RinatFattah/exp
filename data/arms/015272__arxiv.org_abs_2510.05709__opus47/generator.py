...
The attack asks the target model to repeat a single token indefinitely,
inducing divergence and memorised-data leakage. To adapt this template
to arbitrary vanilla harmful prompts, we use the generator LLM to pick
the single most salient noun/topic word from each prompt and wrap it in
one of the six Divergence templates catalogued in Appendix C, Table V.
"""
...
def _estimate_min_memory_gb(model_path: str) -> float:
    """Rough GPU headroom needed to load `model_path` and serve traffic.

    We ask start_server.py to wait for a card that meets this bar instead of
    letting it pick a 17-GB card and OOM on a 27-GB model (see llama.cpp
    `cudaMalloc failed: out of memory` in the failing run's log). Formula:
    on-disk size + 3 GB for KV cache / activations, floored at 12 GB (the
    start_server default) and capped at 140 GB (largest single-GPU we have).
    """
    default = 12.0
    try:
        for candidate in (model_path, os.path.join(os.getcwd(), model_path)):
            if candidate and os.path.isfile(candidate):
                size_gb = os.path.getsize(candidate) / (1024 ** 3)
                return max(default, min(140.0, size_gb + 3.0))
    except OSError:
        pass
    return default
    ...
    async def generate_adversarial_pairs(
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if result.returncode == 0:
                break
            logger.warning(
                "start_server.py exited with %d on attempt %d/%d (min_memory=%.1f GB); retrying in 30s",
                result.returncode, attempt, max_attempts, min_mem_gb,
            )
