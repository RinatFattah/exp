...
Step 1 -- Prompt Rewriting:
    ...
    rewrite loop is retried (up to MAX_ITERATIONS), as in Algorithm 1.
...
Algorithm 1's line-10 check (evaluate the target model's *response* and re-loop
if it isn't harmful) can't be closed here: Phase 3 (`src/generation/run_model.py`)
runs the target model later. We therefore stop after the rewrite-harmfulness
check + nesting, matching the paper's "take the results of the last rewriting
and nesting as the final prompt" fallback.
"""
...
# Scenario nesting templates. "{prompt}" is replaced with the rewritten prompt.
...
class DatasetGenerator:
    ...
    # Algorithm 1 max iterations. Paper uses T = 20, but the response-level
    # loop isn't available in this pipeline, so we keep it modest per sample.
    ...
    def _load_prompts(self, dataset_name: str, column: str) -> List[str]:
        ...
        if dataset_name == "JailbreakBench/JBB-Behaviors":
            ...
            configs = hf_datasets.get_dataset_config_names(
                dataset_name, token=self.hf_token
            )
            for cfg in configs:
                ds_dict = hf_datasets.load_dataset(
                    dataset_name, cfg, token=self.hf_token
                )
        ...
        if column not in ds.column_names:
            raise KeyError(
                f"Column '{column}' not in dataset columns: {ds.column_names}"
            )
    ...
    def _select_candidate(self, text: str) -> str:
        """Pick one candidate at random for the multi-candidate rewrite
        (Paraphrase with Fewer Words), as described in the paper."""
        text = (text or "").strip()
        if text.startswith(CODE_FENCE):
            kept = [
                ln for ln in text.splitlines()
    ...
    async def _apply_rewrites(self, prompt: str, order: List[int]) -> str:
        ...
        for fi in order:
            ...
            try:
                ...
                logger.warning(
                    "Rewrite '%s' failed: %s (keeping previous text)",
                    fn["name"], exc,
                )
                continue
