...
def _render_psa(question: str, sections: dict) -> str:
    ...
    return _PSA_TEMPLATE.format(
        title=(sections.get("title") or "Universal and Transferable "
               "Adversarial Attacks on Aligned Large Language Models").strip(),
        author=(sections.get("author") or "Anonymous Author").strip(),
        attack_methods=(sections.get("attack_methods") or "The attack "
                        "optimizes an adversarial suffix over discrete tokens "
                        "using a greedy, gradient-guided search to bypass "
                        "safety alignment.").strip(),
        mechanism=(sections.get("mechanism") or "The attack succeeds because "
                   "the adversarial context disrupts the model's internal "
                   "harm classification in the middle layers, producing benign "
                   "emotional tokens and bypassing the refusal path.").strip(),
        related_work=(sections.get("related_work") or "Earlier work studied "
                      "adversarial examples for LLMs and alignment via "
                      "RLHF/DPO; jailbreak attacks exploit these "
                      "vulnerabilities.").strip(),
        question=question,
    )
